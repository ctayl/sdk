        ...
        , resizeImage: function (url, options, element, callback) {
            ...
            if(false && root){
                ...
                if (options.size && options.aspect) {
                    if (buildfire.imageLib.ENUMS.SIZES.VALID_SIZES.indexOf(options.size) < 0) {
                        var sizes = buildfire.imageLib.ENUMS.SIZES.VALID_SIZES.join(', ');
                        console.warn('Inavlid size. Availible options are ' + sizes + '. Returning original url');
                        return url;
                    }
                    if (buildfire.imageLib.ENUMS.ASPECT_RATIOS.VALID_RATIOS.indexOf(options.aspect) < 0) {
                        var ratios = buildfire.imageLib.ENUMS.ASPECT_RATIOS.VALID_RATIOS.join(', ');
                        console.warn('Inavlid aspect ratio. Availible options are ' + ratios + '. Returning original url');
                        return url;
                    }
                    //math.round
                    options.width = buildfire.imageLib.ENUMS.SIZES[options.size];
                    options.height = options.width * buildfire.imageLib.ENUMS.ASPECT_RATIOS[options.aspect];
                }
